...
def pdf(x, dof):
    ...
    log.debug("calc t PDF distribution for x=%s and dof=%s" % (x, dof))
    return t.pdf(x, dof)
    ...
    while(True):
        ...
        if not last_result:
            ...
            if E > min_error:
                #last_result=result
                last_result=result
                log.debug("recalculating steps for minimal error")
                #steps = steps * 2
                steps = steps * 2
                if steps > max_steps:
                    log.debug("reach de max number of stpes %s" % steps)
                    break
                log.debug("recalculating steps to %s" % steps)
                w = x / steps
            else:
                log.info("Great!!\n"
                        "Integral:%s \n"
...
def main():
    ...
    if options.t and options.dof:
        if float(options.dof) >= 1:
            result, E, steps = IntegrateSimpsonRule(
                                float(options.t), int(options.dof),
            ...
            parser.error("dof must be greater o equal to one "
